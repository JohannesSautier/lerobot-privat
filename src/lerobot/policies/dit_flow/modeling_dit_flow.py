    ...
    def _start_prefetch(self, batch):
        ...
        def _worker():
            with torch.cuda.stream(self._prefetch_stream), torch.inference_mode():
                actions = self.predict_action_chunk(batch)  # GPU work on separate stream
            # ensure GPU work finished before exposing results
            self._prefetch_stream.synchronize()
            self._prefetched_actions = actions  # (B, n_action_steps, act_dim)
