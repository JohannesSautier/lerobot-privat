...
class DiTFlowPolicy(PreTrainedPolicy):
    ...
    def __init__(
        ...
        # ---- Minimal background prediction state (simple + explicit) ----
        self._pred_thread: threading.Thread | None = None
        self._pred_done = threading.Event()
        self._pred_result: torch.Tensor | None = None  # shape (n_action_steps, B, action_dim) to match extend()
    ...
    def reset(self):
        ...
        # Also reset background prediction state
        # If a thread is running across reset, let it finish then clear results.
        if hasattr(self, "_pred_thread") and self._pred_thread is not None and self._pred_thread.is_alive():
            self._pred_done.wait(timeout=1.0)
    ...
    # ---------- Minimal side-branch helpers ----------
    def _start_prediction_worker(self):
        """
        Start a new prediction thread using the current queues snapshot.
        If a previous worker exists, wait for it to finish before starting a new one.
        """
        # Wait for any previous worker to finish (your "check or wait" requirement)
        if self._pred_thread is not None and self._pred_thread.is_alive():
            self._pred_done.wait()

        # Build the stacked batch (from queues) exactly like predict_action_chunk expects
        batch_stack: dict[str, torch.Tensor] = {}
        for k in self._queues:
            if len(self._queues[k]) > 0:
                batch_stack[k] = torch.stack(list(self._queues[k]), dim=1)

        # Clear the event; launch the worker
        self._pred_done.clear()

        def _worker():
            try:
                with torch.no_grad():
                    actions = self.dit_flow.generate_actions(batch_stack)  # (B, n_action_steps, D)
                    actions = self.unnormalize_outputs({ACTION: actions})[ACTION]
                    # store as (T, B, D) so we can deque.extend(...) without reshaping later
                    self._pred_result = actions.transpose(0, 1)
            except Exception:
                self._pred_result = None
            finally:
                self._pred_done.set()

        self._pred_thread = threading.Thread(target=_worker, daemon=True)
        self._pred_thread.start()

    @torch.no_grad
    def select_action(self, batch: dict[str, torch.Tensor]) -> torch.Tensor:
        """Select a single action given environment observations, with simple side-branch action chunking.

        Behavior:
        - When there are exactly 4 actions left in the buffer, start a background prediction.
        - Before starting, wait for the previous prediction to finish if it's still running.
        - If the action queue runs empty, consume the side-branch result if ready; otherwise, bootstrap synchronously.
        """
        ...
        # Update rolling queues from fresh obs
        self._queues = populate_queues(self._queues, batch)

        # If no actions available, try to use the finished side prediction; else bootstrap once.
        if len(self._queues[ACTION]) == 0:
            # If a worker is running, wait for completion
            if self._pred_thread is not None and self._pred_thread.is_alive():
                self._pred_done.wait()

            if self._pred_result is not None:
                # Extend the queue with finished side-branch result
                # _pred_result shape: (T, B, D) to match prior extend pattern
                self._queues[ACTION].extend(self._pred_result)
                self._pred_result = None
            else:
                # Synchronous one-off prediction to avoid stalling
                actions = self.predict_action_chunk(dict(batch))  # uses queues to stack
                # actions: (B, T, D) -> (T, B, D)
                self._queues[ACTION].extend(actions.transpose(0, 1))

        # When 4 actions remain, kick off the next side prediction (after ensuring old one is done)
        if len(self._queues[ACTION]) == 4:
            if self._pred_thread is not None and self._pred_thread.is_alive():
                self._pred_done.wait()
            self._start_prediction_worker()

        # Pop one action to execute
